...
const meta_data:MetaProps = {
  title:'Kinh đọc hàng ngày',
  description:'Kinh đọc hàng ngày song ngữ Việt-Nhật',
  ogUrl:"",
  ogImage:"/vietcatholicjp-share.jpg"
}

const Index: NextPage = () => {
  const [prayerTypes, setPrayerType] = useState<PrayerType[]>([])
  ...
  useEffect(()=>{
    ...
    let request_endpoint = makeUrl("/api/prayer/")
    if(slug){
      request_endpoint = makeUrl("/api/prayer/"+slug+"/")
    }
    ...
    .then((data) => {
      if(data.status === 'ok'){
        setPrayerType(data.prayer_types)
      }
    })
  },[])
            ...
            <div className="flex w-full">
              <div className="mx-auto flex-auto max-w-4xl min-w-0 pt-6 lg:px-4 lg:pt-8 pb:12 xl:pb-24 lg:pb-16">
                <div className="relative px-2 lg:px-8">
                  ...
                  {prayerTypes?.map((prayer_type,idx)=>(
                    <div className="space-y-4" key={idx}>
                      ...
                      {prayer_type.prayer_type_prayer?.map((prayer,idx)=>(
