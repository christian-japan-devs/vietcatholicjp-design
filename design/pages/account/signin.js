          ...
          <div className='flex items-center justify-center relative mx-auto mt-4 w-full max-w-container grid-cols-1 sm:mt-8 sm:px-6 lg:px-8 xl:mt-16'>
            <div className='p-4 mx-2 w-full max-w-sm shadow-2xl bg-white rounded-lg border border-gray-200 sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700'>
              ...
              <form className="space-y-6 mt-4" onSubmit={handleSubmit}>
                  <div>
                      <label htmlFor="username" className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Tên đăng nhập</label>
                      <input type="text" name="username" id="username" value={username}
                        onChange={(e) =>
                          setUsername(e.target.value)
                        } 
                        className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="Viết liền không dấu" required/>
                  </div>
                  <div>
              ...
              <div className="items-center mt-4 justify-between">
                <div className="text-sm text-center">Hoặc đăng nhập bằng</div>
                <div className="flex items-center justify-between mt-4">
                  {Object.values(providers).map((provider) => {
                    if(provider.name != "Credentials"){
                    return (
                      <div className="flex mx-4 px-6" key={provider.name}>
                        <button onClick={() => signIn(provider.id)} type="button" className="inline-block px-2 sm:px-4 py-2 border-2 border-blue-400 text-gray-800 font-medium text-xs leading-tight uppercase rounded hover:bg-black hover:bg-opacity-5 focus:outline-none focus:ring-0 transition duration-150 ease-in-out">
