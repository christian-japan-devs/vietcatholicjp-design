...
type Props = {
  videoLinks: VideoPostCastType[]
}

const VideoPostCast: NextPage<Props> = ({videoLinks}) => {
    return (
            ...
            <div className="my-4 ms:my-12 relative w-full flex gap-6 snap-x overflow-x-auto pb-8">
              ...
              {videoLinks.map((video,idx)=>(
                <div key={idx} className="snap-center shrink-0 first:pl-8 last:pr-8">
                  <iframe className="shrink-0 w-80 h-56 md:w-96 md:h-64 rounded-lg shadow-xl bg-white" src={"https://www.youtube.com/embed/"+(video.youtube_url)+"?start=1"} title={video.title} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen></iframe>
                </div>
              ))}
              <div className="snap-center shrink-0">
